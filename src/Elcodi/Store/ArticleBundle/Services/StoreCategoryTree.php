...
namespace Elcodi\Store\ArticleBundle\Services;
...
class StoreCategoryTree extends AbstractCacheWrapper
{
     ...
     * @var boolean
     *
     * Load only categories with articles
     */
    private $loadOnlyCategoriesWithArticles;
     ...
     * @param boolean         $loadOnlyCategoriesWithArticles Load only categories with articles
    ...
    public function __construct(
        ...
        $loadOnlyCategoriesWithArticles,
        $key,
        LocaleInterface $locale
    ) {
        $this->categoryTreeService            = $categoryTreeService;
        $this->loadOnlyCategoriesWithArticles = $loadOnlyCategoriesWithArticles;
        $this->key                            = $key;
        $this->locale                         = $locale;
    }
    ...
    protected function formatCategoryNode(array $categoryNode)
    {
        $formatted_node = [
            'entity'   => [
                ...
                'articlesCount' => count($categoryNode['entity']->getPurchasables()),
            ],
     ...
     * the options to only load categories with articles.
     *
    ...
    protected function isCategoryEnabled(Category $category)
    {
        return
            $category->isEnabled() &&
            (
                !$this->loadOnlyCategoriesWithArticles ||
                0 > ($category->getArticles())
            );
    }
